...
public class TabUserDAO {
	private static final Logger log = LoggerFactory.getLogger(TabUserDAO.class);
	// property constants
	...
	public TabUser findById(java.math.BigDecimal id) {
		log.debug("getting TabUser instance with id: " + id);
		try {
			TabUser instance = (TabUser) getCurrentSession().get("dao.TabUser",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(TabUser instance) {
		...
		try {
			List results = getCurrentSession().createCriteria("dao.TabUser")
	...
	public List findByUserdescription(Object userdescription) {
		return findByProperty(USERDESCRIPTION, userdescription);
	}
	...
	public List findByMale(Object male) {
	...
	public TabUser merge(TabUser detachedInstance) {
		...
		try {
			TabUser result = (TabUser) getCurrentSession().merge(
					detachedInstance);
